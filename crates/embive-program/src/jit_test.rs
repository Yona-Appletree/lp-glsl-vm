...
pub fn jit_add_experiment() {
    ...
    println!("[guest] Step 2: Generated instruction encodings - add: 0x{:08x}, ret: 0x{:08x}", add_inst, ret_inst);
    ...
    println!("[guest] Step 3: Generated {} bytes of RISC-V code", riscv_code.len());
    ...
    println!("[guest] Step 4: Created ELF file ({} bytes)", elf_data.len());
    ...
    println!("[guest] Step 5: Allocated {} byte output buffer", OUTPUT_SIZE);
    ...
    println!("[guest] Step 7: Transpiled to {} bytes of embive bytecode", transpiled_size);
    println!("[guest] Step 7: First 16 bytes of transpiled code: {:02x?}", &output_buffer[0..transpiled_size.min(16)]);
    ...
    let add_func: AddFunc = unsafe {
        core::mem::transmute(code_ptr)
    };
    println!("[guest] Step 9: Function pointer created: {:p}", add_func as *const ());
    
    // Call the function with test values
    // output_buffer stays in scope here, so it's safe to call
    let a = 5;
    let b = 10;
    println!("[guest] Step 10: About to call JIT function: add({}, {})", a, b);
