...
pub fn compile_function(func: &lpc_lpir::Function) -> alloc::vec::Vec<u8> {
...
fn compute_max_outgoing_args(func: &lpc_lpir::Function, module: &lpc_lpir::Module) -> usize {
    let mut max_args = 0;
    for block in &func.blocks {
        for inst in &block.insts {
            if let lpc_lpir::Inst::Call { callee, args, .. } = inst {
...
pub struct CompiledModule {
    ...
    pub instructions: alloc::vec::Vec<crate::Inst>,
...
impl CompiledModule {
    ...
    fn format_inst(inst: &crate::Inst) -> alloc::string::String {
        use alloc::format;
        ...
        match inst {
            ...
            crate::Inst::Sltiu { rd, rs1, imm } => {
                format!("sltiu {}, {}, {}", gpr_name(rd), gpr_name(rs1), imm)
            }
            crate::Inst::Xori { rd, rs1, imm } => {
                format!("xori {}, {}, {}", gpr_name(rd), gpr_name(rs1), imm)
            }
            crate::Inst::Ecall => alloc::string::String::from("ecall"),
            crate::Inst::Ebreak => alloc::string::String::from("ebreak"),
        }
    }
    ...
    pub fn function_instructions(&self, function_name: &str) -> Option<&[crate::Inst]> {
        use alloc::vec::Vec;
    ...
    for reloc in relocations {
        ...
        match &reloc.inst_type {
            lower::RelocationInstType::Jal { rd } => {
                let jal_inst = crate::jal(*rd, offset);
...
pub fn compile_module_to_insts(
    ...
    for reloc in &all_relocations {
        ...
        match &reloc.inst_type {
            lower::RelocationInstType::Jal { rd } => {
                all_instructions[inst_idx] = crate::Inst::Jal {
                    rd: *rd,
                    imm: offset,
                };
            }
...
mod tests {
    ...
    fn test_compile_module_with_function_call() {
        ...
        callee_block.push_inst(lpc_lpir::Inst::Iadd {
            result,
            arg1: param,
            arg2: param,
        });
        callee_block.push_inst(lpc_lpir::Inst::Return {
            values: vec![result],
        });
        ...
        let result = Value::new(1);
        caller_block.push_inst(lpc_lpir::Inst::Call {
            callee: String::from("callee"),
            args: vec![param],
            results: vec![result],
        });
        caller_block.push_inst(lpc_lpir::Inst::Return {
            values: vec![result],
        });
...
module {
    ...
    block0:
        ...
        let module = lpc_lpir::parse_module(ir_module).expect("Failed to parse module");
