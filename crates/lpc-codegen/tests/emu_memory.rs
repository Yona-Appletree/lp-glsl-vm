...
use lpc_codegen::{assemble_code, Riscv32Emulator};
...
lw a0, 1(sp)",
    ...
    match result {
        Err(lpc_codegen::EmulatorError::UnalignedAccess {
            address, alignment, ..
        }) => {
...
lw a0, 0x400(sp)",
    ...
    assert!(result.is_err());
    match result {
        Err(lpc_codegen::EmulatorError::InvalidMemoryAccess { address, kind, .. }) => {
            assert_eq!(address, 0x80000000 + 0x400);
            assert_eq!(kind, lpc_codegen::MemoryAccessKind::Read);
        }
...
sw a0, 0x400(sp)",
    ...
    match result {
        Err(lpc_codegen::EmulatorError::InvalidMemoryAccess { address, kind, .. }) => {
            assert_eq!(address, 0x80000000 + 0x400);
            assert_eq!(kind, lpc_codegen::MemoryAccessKind::Write);
        }
        _ => panic!("Expected InvalidMemoryAccess error"),
    }
}
