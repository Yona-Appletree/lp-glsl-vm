//! Emulator runner for executing compiled functions using riscv32-emu.
...
/// Runner for executing compiled functions using the RISC-V 32 emu.
...
impl EmulatorRunner {
    /// Create a new emu runner with default limits.
        ...
        let run_result = loop {
            match emu.step() {
                Ok(StepResult::Halted) => {
                    ...
                    last_result = Some(emu.get_register(crate::Gpr::A0));
                    break Ok(());
                }
                Ok(StepResult::Syscall(syscall_info)) => {
                    ...
                    match syscall_info.number {
                        0 => {
                            ...
                            emu.set_register(crate::Gpr::A0, syscall_info.args[0]);
                        ...
                        1000 => {
                            ...
                            emu.set_register(crate::Gpr::A0, result);
                            // Continue execution
                        }
                        _ => {
