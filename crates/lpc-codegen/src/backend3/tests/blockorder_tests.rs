...
fn test_block_order_critical_edges() {
    ...
    let edge_blocks: Vec<_> = block_order
        .lowered_order
        .iter()
        ...
        match lowered_block {
            ...
            crate::backend3::vcode::LoweredBlock::Edge {
                ...
                if *from == block1 {
                    assert!(
                        found_block1,
                        "Edge block from block1 should come after block1"
                    );
                }
                if *from == block2 {
                    assert!(
                        found_block2,
                        "Edge block from block2 should come after block2"
                    );
                }
            }
        }
...
fn test_entry_block_mapping() {
    ...
    match &block_order.lowered_order[0] {
        crate::backend3::vcode::LoweredBlock::Orig { block } => {
            assert_eq!(
                *block, entry_block,
                "First lowered block should be entry block"
            );
        }
...
fn test_predecessor_computation() {
    ...
    if let Some(pred_range) = vcode.block_pred_range.get(1) {
        let preds: Vec<_> = vcode.block_preds[pred_range.start..pred_range.end]
            .iter()
            .collect();
        assert!(
            preds.contains(&&block0_idx),
